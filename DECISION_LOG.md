...
**Architecture.** I used a modular Next.js/TypeScript application: conversational UI → validated API → minimum-board router → Monday repository/cache → normalization → quality assessment → grounded query planner → deterministic analytics → response composer. Each stage has one responsibility and emits only the contract needed by the next stage.
...
**AI architecture.** I intentionally separated language understanding from metric computation. A deterministic pre-router selects the minimum board set, then a model with strict structured output may determine grounded intent, dimensions, period, and metrics from the live taxonomy. Deterministic TypeScript performs numerical calculations. When no model key exists or planning fails, a typed deterministic planner provides a disclosed fallback. The model cannot run arbitrary GraphQL or invent business data.
...
**Leadership updates.** I interpreted the optional feature as an evidence-backed weekly decision brief: active pipeline, billed revenue, sales exceptions, operational exceptions, leading sector, data reliability, and recommended actions from one snapshot. Copy, regenerate, and Analyst Mode drill-down are included; export was deprioritized.

**Key trade-offs.** I chose a focused full-stack monolith, in-memory TTL cache, transparent rule-based risks, and a custom executive UI. I did not add a database, microservices, complex authentication, arbitrary dashboards, dozens of charts, streaming infrastructure, or decorative 3D. These would reduce reliability and explainability within the assignment time box.
...
**With more time.** I would expand validation across additional Monday schemas, add an admin column-mapping screen, OAuth/tenant isolation, Redis/background refresh, a shared deal/work-order key, planner faithfulness evals, activity fields, accessible chart testing, and PDF/Markdown leadership export.
